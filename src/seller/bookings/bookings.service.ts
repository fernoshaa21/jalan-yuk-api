...
type SellerBookingsPaginationMeta = {
  page: number;
  limit: number;
  total: number;
  totalPages: number;
...
export class SellerBookingsService {
  ...
  async getBookings(
    sellerId: number,
    queryDto: SellerBookingsQueryDto = {},
  ): Promise<{
    data: Array<{
      id: string;
    ...
    meta: SellerBookingsPaginationMeta;
  }> {
    ...
    const query = this.buildSellerBookingsQuery(
      sellerId,
      queryDto,
  ...
  async getBookingById(sellerId: number, id: string) {
    if (!id?.trim()) {
    ...
    const booking = await this.bookingsRepository
      ...
      .andWhere('activity.sellerId = :sellerId', { sellerId })
      .getOne();
  ...
  private buildSellerBookingsQuery(
    sellerId: number,
    queryDto: SellerBookingsQueryDto,
    bookingStatus?: BookingStatus,
    paymentStatus?: PaymentStatus,
  ...
  private normalizePagination(queryDto: SellerBookingsQueryDto): {
    page: number;
    limit: number;
  } {
